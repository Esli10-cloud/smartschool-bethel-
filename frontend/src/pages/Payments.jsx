    ...
    const fetchUserAndData = async () => {
      ...
      try {
        ...
        if (session) {
          ...
          if (profileErr) {
            console.warn("Profil non trouvé dans la table profiles, utilisation par défaut.");
          }

          setCurrentUser({
            id: userId,
            nom: profileData?.full_name || session.user.email || "Utilisateur",
            role: profileData?.role || "secretaire"
          });
        }
  ...
  const getFeeDetails = (student) => {
    ...
    if (isAffected) {
      ...
      if (
        ...
        else if (cls.includes("BAC-PRO")) { tr1 = 140000; tr2 = 40000; tr3 = 40000; baseTotal = 220000; }
      ...
      } else if (cls.includes("3ÈME") || cls.includes("3EME")) { tr1 = 50000; tr2 = 20000; tr3 = 20000; baseTotal = 90000; }
      else if (cls.includes("4ÈME") || cls.includes("4EME")) { tr1 = 40000; tr2 = 20000; tr3 = 20000; baseTotal = 80000; }
      else if (cls.includes("5ÈME") || cls.includes("5EME") || cls.includes("6ÈME") || cls.includes("6EME")) { tr1 = 35000; tr2 = 20000; tr3 = 15000; baseTotal = 70000; }
    }
...
if (uniformSummary) {
  ...
  const handleCancelPayment = async () => {
    ...
    try {
      ...
      await recordAuditLog(
        ...
        `Annulation du reçu/transaction N° ${paymentToCancel.id} d'un montant de ${(paymentToCancel.amount || 0).toLocaleString()} CFA par ${currentUser.nom}. Motif : ${cancelReason}`
      );
  ...
  const generatePDFReceipt = (p) => {
    ...
    doc.text(SCHOOL_CONFIG.name, 74, 12, { align: "center" });
    
    doc.setFont("helvetica", "normal");
    doc.setFontSize(8);
    doc.setTextColor(30, 41, 59);
    doc.text(SCHOOL_CONFIG.subTitle, 74, 16, { align: "center" });
    
    doc.setFont("helvetica", "italic");
    doc.setTextColor(37, 99, 235);
    doc.text(SCHOOL_CONFIG.motto, 74, 20, { align: "center" });

    doc.setFont("helvetica", "normal");
    doc.setFontSize(7.5);
    doc.setTextColor(51, 65, 85);
    doc.text(`Adresse : ${SCHOOL_CONFIG.address} | Tél : ${SCHOOL_CONFIG.phone}`, 74, 25, { align: "center" });
    ...
    const bodyData = [
      ...
      ['Mode de règlement :', p.mode || "Espèces"],
    ...
    doc.text("Toute somme versee n'est plus restituable.", 74, finalY, { align: "center" });

    doc.setFont("helvetica", "bold");
    doc.setTextColor(30, 41, 59);
    doc.text("La Comptabilite", 110, finalY + 12, { align: "center" });

    doc.save(`Recu_Paiement_${p.id}.pdf`);
  };
  ...
  const classesList = ["Tous", ...new Set(students.map(s => s.classe ? s.classe.trim().toUpperCase() : "").filter(Boolean))];
  ...
  const filteredPayments = payments.filter((p) => {
    ...
    const pClasse = p.students?.classe ? p.students.classe.trim().toUpperCase() : "";
    const matchClass = filterClassHistory === "Tous" || pClasse === filterClassHistory;
  ...
  const chartData = Object.entries(statsParMode).map(([mode, montant]) => ({
    name: mode,
    montant: montant
  }));
  ...
  const filteredImpayes = listeImpayes.filter(s => {
    const sClasse = s.classe ? s.classe.trim().toUpperCase() : "";
    const matchClass = filterClassImpayes === "Tous" || sClasse === filterClassImpayes;
                      ...
                      currentDisplayedPayments.map((p) => {
                        ...
                        return (
                          <tr key={p.id} style={{ borderBottom: "1px solid #f1f5f9", fontSize: "14px", opacity: p.is_cancelled ? 0.6 : 1, background: p.is_cancelled ? "#fef2f2" : "white" }}>
                            ...
                            <td style={{ padding: "14px 20px", color: "#64748b" }}>{p.created_at ? new Date(p.created_at).toLocaleDateString("en-GB") : "-"}</td>
                            ...
                            <td style={{ padding: "14px 20px", textAlign: "right" }}>
                              <div style={{ display: "flex", gap: "6px", justifyContent: "flex-end" }}>
                                {!p.is_cancelled && (
                                  <>
                                    <button onClick={() => { setSelectedStudentId(p.student_id); setPayInscription(true); setPayPaperRame(false); setPayDortoir(false); setIsModalOpen(true); }} style={{ background: "#f0fdf4", color: "#16a34a", border: "1px solid #bbf7d0", padding: "6px 10px", borderRadius: "6px", fontWeight: "600", cursor: "pointer", fontSize: "12px" }}>
          ...
          {activeTab === "dashboard" && (
            <div>
              ...
              <div style={{ background: "white", padding: "24px", borderRadius: "12px", border: "1px solid #e2e8f0", marginBottom: "20px" }}>
                <h3 style={{ marginBottom: "16px", fontSize: "16px", color: "#1e293b" }}>Évolution des encaissements par mode de règlement</h3>
                <div style={{ width: "100%", height: "260px" }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={chartData}>
                      ...
                      <XAxis dataKey="name" />
                      <YAxis />
                      <Tooltip formatter={(value) => `${value.toLocaleString()} CFA`} />
              ...
              <div style={{ background: "white", padding: "24px", borderRadius: "12px", border: "1px solid #e2e8f0" }}>
                <h3 style={{ marginBottom: "16px", fontSize: "16px" }}>Détails par Mode de Règlement</h3>
                        ...
                        return (
                          <tr key={st.id} style={{ borderBottom: "1px solid #f1f5f9", fontSize: "14px" }}>
                            ...
                            <td className="no-print" style={{ padding: "14px 20px", textAlign: "right" }}>
                              <button onClick={() => { setSelectedStudentId(st.id); setPayInscription(true); setPayPaperRame(false); setPayDortoir(false); setIsModalOpen(true); }} style={{ background: "#eff6ff", color: "#2563eb", border: "1px solid #bfdbfe", padding: "6px 12px", borderRadius: "6px", fontWeight: "600", cursor: "pointer", fontSize: "13px" }}>
                                Encaisser
                              </button>
                            </td>
                          </tr>
              ...
              <div id="caisse-print" style={{ background: "white", padding: "24px", borderRadius: "12px", border: "1px solid #e2e8f0" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
                  ...
                  <div style={{ textAlign: "right", fontSize: "13px" }}>
                    <div><strong>Date :</strong> {new Date(caisseDate).toLocaleDateString("en-GB")}</div>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  ...
                  <tbody>
                    {paymentsDuJour.length === 0 ? (
                      <tr><td colSpan="7" style={{ padding: "20px", textAlign: "center", color: "#64748b" }}>Aucun versement enregistré à cette date.</td></tr>
                    ) : (
          ...
          {activeTab === "audit" && currentUser.role === "admin" && (
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
                ...
                <button onClick={() => window.location.reload()} style={{ background: "#7c3aed", color: "white", border: "none", padding: "8px 14px", borderRadius: "8px", fontWeight: "600", cursor: "pointer" }}>
                  🔄 Actualiser les logs
                </button>
              </div>
                    ...
                    {auditLogs.length === 0 ? (
                      ...
                      auditLogs.map((log) => (
                        <tr key={log.id} style={{ borderBottom: "1px solid #f1f5f9", fontSize: "13px" }}>
                          <td style={{ padding: "14px 20px", color: "#64748b" }}>{log.created_at ? new Date(log.created_at).toLocaleString("en-GB") : "-"}</td>
...
{/* Affichage forcé des 3 tranches d'échéance */}
{(() => {
  // On récupère l'objet élève par son ID ou on prend le premier si besoin pour tester
  const currentStudent = (students || []).find(s => String(s.id) === String(selectedStudentId)) || (students && students[0]);
  const feeInfo = currentStudent ? getFeeDetails(currentStudent) : { baseTotal: 100000, installments: { v1: 40000, v2: 30000, v3: 30000 } };
  const dejaPaye = typeof totalDejaPaye !== 'undefined' ? totalDejaPaye : 0;

  return (
    <div style={{ backgroundColor: '#eff6ff', border: '2px solid #3b82f6', borderRadius: '8px', padding: '12px', marginTop: '12px', marginBottom: '12px', fontSize: '12px' }}>
      <p style={{ fontWeight: '700', color: '#1e40af', marginBottom: '8px' }}>Échéancier officiel des 3 tranches :</p>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px', textAlign: 'center' }}>
        
        {/* 1er Versement */}
        <div style={{ padding: '8px', borderRadius: '6px', border: '1px solid #93c5fd', backgroundColor: dejaPaye >= (feeInfo.installments?.v1 || 40000) ? '#dcfce7' : '#ffffff' }}>
          ...
          <span style={{ fontWeight: '600' }}>{(feeInfo.installments?.v1 || 40000).toLocaleString()} F</span>
        </div>

        {/* 2ème Versement */}
        <div style={{ padding: '8px', borderRadius: '6px', border: '1px solid #93c5fd', backgroundColor: dejaPaye >= ((feeInfo.installments?.v1 || 40000) + (feeInfo.installments?.v2 || 30000)) ? '#dcfce7' : '#ffffff' }}>
          ...
          <span style={{ fontWeight: '600' }}>{(feeInfo.installments?.v2 || 30000).toLocaleString()} F</span>
        </div>

        {/* 3ème Versement */}
        <div style={{ padding: '8px', borderRadius: '6px', border: '1px solid #93c5fd', backgroundColor: dejaPaye >= (feeInfo.baseTotal || 100000) ? '#dcfce7' : '#ffffff' }}>
          ...
          <span style={{ fontWeight: '600' }}>{(feeInfo.installments?.v3 || 30000).toLocaleString()} F</span>
        </div>
        ...
        <Modal isOpen={receiptModalOpen} onClose={() => setReceiptModalOpen(false)} title="">
          <div id="receipt-a5" style={{ padding: "15px", color: "#0f172a", fontFamily: "sans-serif", maxWidth: "650px", margin: "0 auto", background: "white" }}>
            ...
            {(() => {
              ...
              return (
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4px 10px", fontSize: "10.5px", background: "#f8fafc", padding: "6px 10px", borderRadius: "6px", border: "1px solid #e2e8f0", marginBottom: "8px" }}>
                  <div><strong>Date :</strong> {selectedReceipt.created_at ? new Date(selectedReceipt.created_at).toLocaleDateString("en-GB") : "-"}</div>
            ...
            <div style={{ textAlign: "center", margin: "10px 0", padding: "6px", background: "#f8fafc", borderRadius: "6px", border: "1px solid #e2e8f0" }}>
              <span style={{ fontSize: "10.5px", fontWeight: "700", color: "#b91c1c", letterSpacing: "0.3px", textTransform: "uppercase" }}>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", marginTop: "10px" }}>
              <div style={{ textAlign: "center", minWidth: "130px" }}>
                <div style={{ fontWeight: "700", color: "#1e293b", textTransform: "uppercase", fontSize: "9.5px" }}>
                  La Comptabilité
                </div>
              </div>
            </div>
            ...
            <div className="no-print" style={{ display: "flex", justifyContent: "flex-end", gap: "10px", marginTop: "12px" }}>
              ...
              <button onClick={() => generatePDFReceipt(selectedReceipt)} style={{ padding: "6px 14px", borderRadius: "6px", border: "none", background: "#16a34a", color: "white", fontWeight: "600", cursor: "pointer", display: "inline-flex", alignItems: "center", gap: "6px" }}>
